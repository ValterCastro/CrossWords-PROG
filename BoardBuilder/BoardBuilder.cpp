...
int main() {
  ...
  while (true) {
    ...
    while (true) {
      Word word = Requester::requestWord(acceptableWords, board.getNrRows(),
                                         board.getNrCollumns());
    ...
    if (board.getWords().size() >= MIN_WORDS &&
        Requester::requestIsBoardFinished())
      break;
  }

  FileWrapper::saveBoardToFile(board);
  // TODO: move .txt to a board.getFilename()
