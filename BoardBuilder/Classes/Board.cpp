...
bool Board::checkHorizontal(Word word, unsigned short rowIndex, unsigned short collumnIndex){
	char tile;
	
	for(size_t i = 0; i < word.getWord().size(); i++){
		if(this->getNrCollumns() <= (collumnIndex + i))
			return false;

		tile = this->tiles[rowIndex][collumnIndex + i];
	
		if(tile != '\0' && tile != word.getWord()[i])
			return false;
	}
...
bool Board::checkVertical(Word word, unsigned short rowIndex, unsigned short collumnIndex){
	char tile;
	
	for(size_t i = 0; i < word.getWord().size(); i++){
		if(this->getNrRows() <= (rowIndex + i))
			return false;
